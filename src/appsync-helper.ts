...
export type UnitResolverFile = BaseResolver & {
  kind: 'UNIT';
  dsName: string;
};

export type PipelineResolverGroup = BaseResolver & {
  kind: 'PIPELINE';
  hasCode?: boolean;
  fns: AppSyncFn[];
};

export type AppSyncFn = {
  name: string;
  description?: string;
  key: string;
  dsName: string;
...
export class AppSyncHelper extends GraphqlApiBase {
  ...
  constructor(scope: Construct, id: string, props: AppSyncHelperProps) {
    super(scope, id);
    this.modes = [AuthorizationType.USER_POOL];
  ...
  getPipelineResolvers() {
    ...
    const FN_LOC_REG = /^(?<order>\d+)\.(?<description>\w+)\.\[(?<ds>\w+)\]\.(ts)$/;
  ...
  addIAMHttpDataSource(id: string, endpoint: string, options?: HttpDataSourceOptions & { serviceRoleArn: string }) {
    const ds = new CfnDataSource(this, id, {
      apiId: this.apiId,
      name: id,
      type: 'HTTP',
      ...
      httpConfig: {
        ...
        authorizationConfig: {
          ...
          awsIamConfig: {
            signingRegion: options?.authorizationConfig?.signingRegion || '',
            signingServiceName: options?.authorizationConfig?.signingServiceName || '',
          }
        }
      }
    });
    this.addCfnDataSource(ds);
    return ds;
  }
